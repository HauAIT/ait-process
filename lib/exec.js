...
async function exec (cmd, args = [], opts = /** @type {T} */({})) {
  ...
  return await new B((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      ...
      if (code === 0) {
        resolve(/** @type {BufferProp<T> extends true ? ExecBufferResult : ExecStringResult} */({stdout, stderr, code}));
      } else {
 ...
 * @typedef {import('child_process').SpawnOptions & AITProcessProps} ExecOptions
 */

/**
 * The value {@link exec ait-process.exec} resolves to when `isBuffer` is `false`
 * @typedef {Object} ExecStringResult
 ...
 * @typedef {Object} ExecBufferResult
 ...
 * @typedef {Error & ExecErrorProps} ExecError
 */
